#!/usr/bin/env bash
# Zerodha BOPS: 10 sequential no-cache timing runs, same stats shape as run_bench_x10.sh.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
OUT="$REPO_ROOT/guides/cursor/baselines/zerodha_bops_x10_wsl"
mkdir -p "$OUT"

export BENCH_ITERATIONS="${BENCH_ITERATIONS:-5000}"
export BENCH_WORKERS="${BENCH_WORKERS:-48}"

echo "Zerodha BOPS benchmark x10 (sequential): iterations=$BENCH_ITERATIONS workers=$BENCH_WORKERS"
echo "Output: $OUT"

cd "$(dirname "${BASH_SOURCE[0]}")"
for i in $(seq 1 10); do
  echo "=== BOPS Run $i / 10 ==="
  go run . 2>&1 | tee "$OUT/zerodha_bops_run${i}.txt"
done

STATS="$REPO_ROOT/guides/cursor/baselines/zerodha_bops_x10_wsl_stats_latest.txt"
...
runs = []
for path in sorted(out.glob("zerodha_bops_run*.txt"), key=lambda p: int(re.search(r"(\d+)", p.stem).group(1))):
    text = path.read_text()
...
lines = [
    "# Zerodha BOPS x10 latest summary (bypass-cache, no content reuse)",
    "",
    f"Runs: {len(runs)}",
]
if runs:
