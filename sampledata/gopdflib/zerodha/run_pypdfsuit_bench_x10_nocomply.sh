#!/usr/bin/env bash
# PyPDFSuit Zerodha Gold Standard (non-compliant): 5000 iterations × 10 sequential timing runs.
# Same 80/15/5 workload with PDF/A, tagging, signing, and font embedding disabled.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
OUT="$REPO_ROOT/guides/cursor/baselines/pypdfsuit_bench_x10_nocomply_wsl"
mkdir -p "$OUT"
...
echo "PyPDFSuit Zerodha non-compliant benchmark ×10 (sequential): iterations=$BENCH_ITERATIONS workers=$BENCH_WORKERS"
echo "Output: $OUT"

cd "$(dirname "${BASH_SOURCE[0]}")"
for i in $(seq 1 10); do
  echo "=== Run $i / 10 ==="
  python3 pypdfsuit_bench.py 2>&1 | tee "$OUT/pypdfsuit_nocomply_run${i}.txt"
done

STATS="$REPO_ROOT/guides/cursor/baselines/pypdfsuit_bench_x10_nocomply_wsl_stats_latest.txt"
...
runs = []
for path in sorted(out.glob("pypdfsuit_nocomply_run*.txt"), key=lambda p: int(re.search(r"(\d+)", p.stem).group(1))):
    text = path.read_text()
...
lines = [
    "# PyPDFSuit Zerodha x10 non-compliant latest summary",
    "",
    f"Runs: {len(runs)}",
]
if runs:
