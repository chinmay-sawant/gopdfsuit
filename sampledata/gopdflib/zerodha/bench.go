// Shared Zerodha gold-standard benchmark for gopdflib (compliant and non-compliant).
...
func basePDFConfig(pdfTitle, watermark string) gopdflib.Config {
	cfg := gopdflib.Config{
		PageBorder:    "0:0:0:0",
		Page:          "A4",
		PageAlignment: 1,
	...
	if benchCompliant {
		cfg.PDFACompliant = true
		cfg.TaggedPDF = true
		cfg.ArlingtonCompatible = true
		cfg.EmbedFonts = boolPtr(true)
	}
	return cfg
}
...
func buildRetailTemplate() gopdflib.PDFTemplate {
	retailCfg := basePDFConfig("Contract Note - CN2024001", "")
	if benchCompliant {
...
func runBenchmark() error {
	...
	if benchCompliant {
		fmt.Println("Mode: compliant (PDF/A-4, PDF/UA-2, tagged, signed retail)")
	} else {
		fmt.Println("Mode: non-compliant (PDF/A, tagging, Arlington, embed-fonts, signing off)")
	}
	...
	if !skipWrite {
		...
		for name, data := range map[string][]byte{
			outputPDFName("zerodha_retail_output.pdf"): retailPDF,
			outputPDFName("zerodha_active_output.pdf"): activePDF,
			outputPDFName("zerodha_hft_output.pdf"):    hftPDF,
		} {
