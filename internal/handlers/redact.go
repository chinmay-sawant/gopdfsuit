...
func normalizeTextSearchQueries(queries []redact.RedactionTextQuery) []redact.RedactionTextQuery {
	if len(queries) == 0 {
		return nil
	}
	seen := make(map[string]struct{}, len(queries))
	normalized := make([]redact.RedactionTextQuery, 0, len(queries))
	for _, q := range queries {
		for _, term := range parseCommaSeparatedTerms(q.Text) {
			...
			seen[key] = struct{}{}
			normalized = append(normalized, redact.RedactionTextQuery{Text: term})
		}
	}
	return normalized
}
...
func HandleRedactPageInfo(c *gin.Context) {
	...
	info, err := redact.GetPageInfo(pdfBytes)
	if err != nil {
...
func HandleRedactCapabilities(c *gin.Context) {
	...
	caps, err := redact.AnalyzePageCapabilities(pdfBytes)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func HandleRedactTextPositions(c *gin.Context) {
	...
	positions, err := redact.ExtractTextPositions(pdfBytes, pageNum)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func HandleRedactApply(c *gin.Context) {
	...
	var options redact.ApplyRedactionOptions
	...
	if textSearchJSON != "" {
		if err := json.Unmarshal([]byte(textSearchJSON), &options.TextSearch); err != nil {
			...
			for _, text := range plain {
				...
				options.TextSearch = append(options.TextSearch, redact.RedactionTextQuery{Text: text})
			}
		}
	}
	...
	if len(options.TextSearch) == 0 {
		if searchText := strings.TrimSpace(c.PostForm("text")); searchText != "" {
			...
			options.TextSearch = make([]redact.RedactionTextQuery, 0, len(terms))
			for _, t := range terms {
				options.TextSearch = append(options.TextSearch, redact.RedactionTextQuery{Text: t})
			}
		}
	}
	...
	redactedPDF, report, err := redact.ApplyRedactionsAdvancedWithReport(pdfBytes, options)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func HandleRedactSearch(c *gin.Context) {
	...
	rects, err := redact.FindTextOccurrencesMulti(pdfBytes, terms)
	if err != nil {
