    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.elements && Array.isArray(data.elements)) {
          ...
          combinedComponents = data.elements.map(el => {
            if (el.type === 'table' && data.table && data.table[el.index]) {
              return { ...data.table[el.index], type: 'table' }
            } else if (el.type === 'spacer' && data.spacer && data.spacer[el.index]) {
              return { ...data.spacer[el.index], type: 'spacer' }
            } else if (el.type === 'image' && data.image && data.image[el.index]) {
              return { ...data.image[el.index], type: 'image' }
            }
            return null
          }).filter(Boolean)
        } else {
          // Fallback for templates without elements array (legacy behavior)
          if (data.table) {
