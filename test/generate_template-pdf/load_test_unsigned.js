...
// Same shape as load_test.js; payloads are unsigned (no signature) and non-PDF/A (tagging off).
...
const unsignedOpts = getPayloadOptions('unsigned');

export default function () {
    const url = `${BASE_URL}/api/v1/generate/template-pdf`;
    const payload = JSON.stringify(getWeightedPayload(unsignedOpts));
...
export function setup() {
    console.log(`Starting UNSIGNED load test against ${BASE_URL}`);
    console.log('Scenario: pdfaCompliant=false, taggedPDF=false, sign=false');
...
export function teardown(data) {
    console.log(`Unsigned load test completed. Started at: ${data.startTime}`);
