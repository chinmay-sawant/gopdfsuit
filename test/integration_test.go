...
// TestGenerateTemplatePDFThenFillWithXFDF chains /api/v1/generate/template-pdf
// and /api/v1/fill through real HTTP. Validates the contract between the template
// generator (which produces AcroForm fields) and the XFDF filler (which expects
// them in a specific internal format). Covers Calisoft Lab2 Test 02.
func (s *IntegrationSuite) TestGenerateTemplatePDFThenFillWithXFDF() {
	// 1. Generate a PDF from a template that declares a first_name form field.
	jsonPath := filepath.Join("..", "sampledata", "acroform", "us_patient_healthcare_form_og.json")
	jsonData, err := os.ReadFile(jsonPath)
	s.NoError(err, "Failed to read patient template JSON")

	genResp, err := s.client.Post(s.ts.URL+"/api/v1/generate/template-pdf", "application/json", bytes.NewBuffer(jsonData))
	s.NoError(err)
	defer func() {
		_ = genResp.Body.Close()
	}()
	s.Equal(http.StatusOK, genResp.StatusCode)
	s.Equal("application/pdf", genResp.Header.Get("Content-Type"))

	pdfBytes, err := io.ReadAll(genResp.Body)
	s.NoError(err)
	s.Greater(len(pdfBytes), 0, "Generated PDF should not be empty")
	s.True(bytes.HasPrefix(pdfBytes, []byte("%PDF")), "Generated bytes should start with %PDF")

	// Persist the pre-fill PDF so the lab report can show an apples-to-apples
	//before/after screenshot (same pipeline, only the field value differs)

	s.NoError(os.WriteFile(
		filepath.Join("..", "sampledata", "acroform", "temp_generated_only.pdf"),
		pdfBytes, 0644))

	// 2. Build an XFDF that sets first_name to TestNombre.
	xfdfBytes := []byte(`<?xml version="1.0" encoding="UTF-8"?>
<xfdf xmlns="http://ns.adobe.com/xfdf/" xml:space="preserve">
 <fields>
   <field name="first_name"><value>TestNombre</value></field>
 </fields>
</xfdf>`)

	// 3. POST both to /api/v1/fill as multipart form-data.
	body := &bytes.Buffer{}
	writer := multipart.NewWriter(body)

	pdfPart, err := writer.CreateFormFile("pdf", "generated.pdf")
	s.NoError(err)
	_, err = pdfPart.Write(pdfBytes)
	s.NoError(err)

	xfdfPart, err := writer.CreateFormFile("xfdf", "data.xfdf")
	s.NoError(err)
	_, err = xfdfPart.Write(xfdfBytes)
	s.NoError(err)

	s.NoError(writer.Close())

	fillResp, err := s.client.Post(s.ts.URL+"/api/v1/fill", writer.FormDataContentType(), body)
	s.NoError(err)
	defer func() {
		_ = fillResp.Body.Close()
	}()
	s.Equal(http.StatusOK, fillResp.StatusCode)
	s.Equal("application/pdf", fillResp.Header.Get("Content-Type"))

	filledBytes, err := io.ReadAll(fillResp.Body)
	s.NoError(err)
	s.Greater(len(filledBytes), 0, "Filled PDF should not be empty")

	// 4. Persist the artifact as evidence for the lab report.
	outPath := filepath.Join("..", "sampledata", "acroform", "temp_generate_then_fill.pdf")
	s.NoError(os.WriteFile(outPath, filledBytes, 0644))

	// 5. The value must be in the filled PDF, and it must sit inside the
	// AcroForm /V entry — proves the round-trip across both subsystems works.
	s.True(bytes.Contains(filledBytes, []byte("TestNombre")),
		"Filled PDF should contain the string 'TestNombre'")
	s.True(bytes.Contains(filledBytes, []byte("/V (TestNombre)")),
		"Filled PDF should contain '/V (TestNombre)' so the value lands in the AcroForm field")
}
...
// TestMergePreservesUserOrder — TC01
// Verifica que el orden de archivos enviado al endpoint /api/v1/merge
// (que refleja el orden elegido por el usuario en el frontend) se respeta
// en el PDF resultante. Se suben 3 PDFs con marcas MARK_A, MARK_B, MARK_C
// en orden C, A, B y se verifica que el PDF descargado tiene ese mismo orden de páginas.
func (s *IntegrationSuite) TestMergePreservesUserOrder() {
	pdfA := createMinimalPDF("MARK_A")
	pdfB := createMinimalPDF("MARK_B")
	pdfC := createMinimalPDF("MARK_C")

	// El usuario reordena en el frontend: quiere orden C, A, B
	orderedFiles := []struct {
		name string
		data []byte
	}{
	...
	for _, f := range orderedFiles {
		part, err := writer.CreateFormFile("pdf", f.name)
		s.NoError(err)
	...
	mergedPDF, err := io.ReadAll(resp.Body)
	s.NoError(err)
	s.NotEmpty(mergedPDF)

	// Guardar PDF resultante para inspección manual (evidencia T03)
	baseDir := filepath.Join("..", "sampledata", "merge")
	tempPath := filepath.Join(baseDir, "temp_tc01_merge_order.pdf")
	_ = os.WriteFile(tempPath, mergedPDF, 0644)

	posC := bytes.Index(mergedPDF, []byte("MARK_C"))
	posA := bytes.Index(mergedPDF, []byte("MARK_A"))
	posB := bytes.Index(mergedPDF, []byte("MARK_B"))

	s.True(posC >= 0, "MARK_C no encontrado en el PDF combinado")
	s.True(posA >= 0, "MARK_A no encontrado en el PDF combinado")
	s.True(posB >= 0, "MARK_B no encontrado en el PDF combinado")
	s.True(posC < posA, "página 1 debe ser MARK_C (posición C=%d, A=%d)", posC, posA)
	s.True(posA < posB, "página 2 debe ser MARK_A (posición A=%d, B=%d)", posA, posB)
}

// createMinimalPDF genera un PDF válido de 1 página con el texto dado embebido
// en el content stream sin compresión, para que sea buscable con bytes.Index.
func createMinimalPDF(mark string) []byte {
	content := fmt.Sprintf("BT /F1 12 Tf 100 700 Td (%s) Tj ET", mark)
	streamLen := len(content) + 1
	...
	buf.WriteString(fmt.Sprintf("0 6\n"))
